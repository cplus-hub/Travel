---
title: "Day 1: 沖繩我來了！"
date: 2026-05-13
weight: 2
---
1.抵達機場 -> 2.取車 -> 3.午餐 泊港漁市場 ->  
4.PARCO CITY 購物 -> 5.美國村 -> 6.殘波岬 -> 7.萬座毛 ->  
8.飯店 - KARIYUSHI LCH.RESORT on The Beach


<!--more-->

<div class="w-100 ph2 mb4">
    <img src="/Travel/images/departure.jpg" class="db w-100 br2 shadow-4" alt="機票">
    <h3 class="f4 mv2">飛機出發時間 - 6:50(台灣時間)</br>
    航&nbsp;&nbsp;&nbsp;行&nbsp;&nbsp;時&nbsp;&nbsp;間&nbsp; - 1:30</br>
    預計抵達時間 - 9:20(日本時間)
    </h3>
</div>

<div class="flex flex-wrap -mx2 mt4">
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_airport.jpg" class="db w-100 br2 shadow-4" alt="機場">
        <h3 class="f4 mv2">抵達機場</h3>
        <p class="lh-copy f6">在機場逛逛，完成紀念品明信片遊戲。</p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_getcar.jpg" class="db w-100 br2 shadow-4" alt="取車">
        <h3 class="f4 mv2">搭電車至赤嶺站取車（與車行約11點）</h3>
        <p class="lh-copy f6"></p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_lunch.jpg" class="db w-100 br2 shadow-4" alt="泊港漁市場">
        <h3 class="f4 mv2">午餐時間 - 泊港漁市場</h3>
        <p class="lh-copy f6"></p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_shopping.jpg" class="db w-100 br2 shadow-4" alt="PARCO CITY">
        <h3 class="f4 mv2">景點 - PARCO CITY 購物</h3>
        <p class="lh-copy f6">可以買買買</p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_american.jpg" class="db w-100 br2 shadow-4" alt="美國村">
        <h3 class="f4 mv2">景點 - 美國村</h3>
        <p class="lh-copy f6">到美國了哦！</p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_coastal.jpg" class="db w-100 br2 shadow-4" alt="殘波岬">
        <h3 class="f4 mv2">景點 - 殘波岬</h3>
        <p class="lh-copy f6">看海</p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_coastal2.jpg" class="db w-100 br2 shadow-4" alt="萬座毛">
        <h3 class="f4 mv2">景點 - 萬座毛</h3>
        <p class="lh-copy f6">看海</p>
    </div>
    <div class="w-100 w-50-ns ph2 mb4">
        <img src="/Travel/images/Day1_hotel.jpg" class="db w-100 br2 shadow-4" alt="飯店">
        <h3 class="f4 mv2">住宿飯店 - KARIYUSHI LCH.RESORT on The Beach</h3>
        <p class="lh-copy f6">好好休息，躺躺</p>
    </div>
</div>

[🔙 回行程總覽](../)

[📥 下載本頁 PDF](/Travel/pdf/20260513_Okinawa_Day1.pdf)